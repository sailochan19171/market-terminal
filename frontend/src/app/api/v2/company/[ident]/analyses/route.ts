...
export const GET = handle(async (req: Request, ctx: Ctx<{ ident: string }>) => json(companyAnalysesList(db(), decodeURIComponent((await ctx.params).ident), Args.of(req)), { shared: false }));
